...
from model import Task3ElementModel
import data
import utils


def load_model(params):
    return Task3ElementModel(params, 19) # BIOC 
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(iter_):
            ...
            input_ids, token_type_ids, attention_mask, labels, fact_labels = batch
...
def main(params):
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            batch = tuple(t.to(device) for t in batch)
            input_ids, token_type_ids, attention_mask, tag_labels, fact_labels = batch
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_path', type=str, default='./data/model/task3_extractive_element')
