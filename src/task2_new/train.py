...
def load_model(params, model_index):
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(iter_):
            ...
            correct_num, all_num = utils.classification_accuracy(prediction, labels, model_index)
...
def train(
    ...
    model_output_path = os.path.join(model_base_path, 'module_%d' %model_index)
    module_names = ['tag_0', 'tag_1', 'tag_2', 'type']
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            ...
            if (step + 1) % (params["print_interval"] * grad_acc_steps) == 0:
                if (logger):
                    logger.info(
                        "Module %s, Step %d - epoch %d average loss: %.4f; loss: %.4f" %(
                            module_names[model_index],
                            step,
                            epoch_idx,
        ...
        if (params['do_evaluate']):
            ...
            results = evaluate(
                model, valid_dataloader, device=device, logger=logger, model_index=model_index
            )
    ...
    if (params['do_evaluate']):
        ...
        model = load_model(params, model_index)
        model.to(model.device)
        utils.save_model(model, tokenizer, model_output_path)

        if params["final_evaluate"]:
            params["load_model_path"] = model_output_path
            evaluate(model, valid_dataloader, device=device, logger=logger, model_index=model_index)
...
def main(params):
    ...
    train_type_dataset, train_tag_datasets = data.process_data(
        train_samples,
        tokenizer,
        params,
    )
    ...
    if (params['do_evaluate']):
        ...
        valid_type_dataset, valid_tag_datasets = data.process_data(
            valid_samples,
            tokenizer,
            params,
        )
    ...
    for i in range(3):
        train(
            params,
            train_tag_datasets[i],
            valid_tag_datasets[i],
            logger,
            i,
            model_output_path,
        ) # train tag model
